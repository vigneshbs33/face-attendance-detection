...
class FaceAttendanceApp {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('nameInput').addEventListener('input', (e) => {
            document.getElementById('registerFace').disabled = !e.target.value.trim() || !this.capturedImage;
        });
    }
    ...
    async registerFace() {
        ...
        if (!name || !this.capturedImage) {
            this.showNotification('Please enter a name and capture a photo.', 'error');
            return;
        }
        
        const result = await this.sendApiRequest('/api/register', 'POST', { name: name, image: this.capturedImage });
        if (result.success) {
